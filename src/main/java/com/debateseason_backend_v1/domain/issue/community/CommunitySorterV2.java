...
@Setter
@Component
public class CommunitySorterV2 { // 자주 들락날락할텐데 DB에 기록하면 매우느림. 그리고 나중에 사용자 많아지면, OOM발생할거 같은데.
	...
	public CommunitySorterV2(CommunityList communityList) {
	...
	public void record(UserDTO userDTO, Long issueId) {
		...
		if(usersByIssue.get(issueId) != null){ // 1. 이미 만들어진 이슈방에 대해서( 이슈방은 이미 1명 이상의 사용자를 가지고 있다. ) <- 이 부분이 꺼림직 함.
			...
			synchronized (this){
				...
				// Double checking Lock을 통해서 이미 생성이 되었다면, 앞의 if문이 코드를 실행하게 하자.
	...
	private synchronized void checkUser(UserDTO userDTO, Long issueId){
