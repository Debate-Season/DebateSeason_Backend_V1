...
public class ChatApiTest {
	...
	void 채팅메시지_조회_성공() throws Exception {
		...
		headers.add("Authorization", "Bearer " + jwtUtil.issueTokenPair(new UserId(userId)).accessToken());
	...
	private String createTestJwt(Long userId) {
		return jwtUtil.issueTokenPair(new com.debateseason_backend_v1.domain.user.domain.UserId(userId)).accessToken();
	}
