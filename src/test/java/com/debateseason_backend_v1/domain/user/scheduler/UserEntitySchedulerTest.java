...
class UserEntitySchedulerTest {
	...
	//
	// 	UserEntity userEntity1 = createUser(1L, "old-id-1", true, now.minusDays(6));
	// 	UserEntity userEntity2 = createUser(2L, "old-id-2", true, now.minusDays(7));
	// 	List<UserEntity> expiredUserEntities = Arrays.asList(userEntity1, userEntity2);
	//
	...
	// 		.thenReturn(expiredUserEntities);
	...
	//
	// 	assertAll(
	// 		() -> assertEquals("short-1", userEntity1.getIdentifier()),
	// 		() -> assertEquals("short-2", userEntity2.getIdentifier()),
	...
	// 	LocalDateTime cutoffDate = now.minusDays(5);
	//
	// 	UserEntity userEntity = createUser(1L, "old-id", true, now.minusDays(6));
	// 	List<UserEntity> expiredUserEntities = Collections.singletonList(userEntity);
	//
	// 	when(userRepository.findByIsDeletedTrueAndUpdatedAtBefore(any(LocalDateTime.class)))
	// 		.thenReturn(expiredUserEntities);
	...
	// 	assertEquals("short-uuid", userEntity.getIdentifier());
	// }
	//
	// @Test
	...
	private UserEntity createUser(Long id, String identifier, boolean isDeleted, LocalDateTime updatedAt) {
		...
		ReflectionTestUtils.setField(userEntity, "isDeleted", isDeleted);
		ReflectionTestUtils.setField(userEntity, "updatedAt", updatedAt);
